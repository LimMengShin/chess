...
def evaluation(board):
    scoring = {'p': -100, 'n': -320, 'b': -330, 'r': -500, 'q': -900, 'k': -20000, 'P': 100, 'N': 320, 'B': 330, 'R': 500, 'Q': 900, 'K': 20000}
    score = 0
    end_game = check_end_game(board)
    pieces = board.piece_map()
    for square, piece in pieces.items():
        score += scoring[str(piece)]
        if board.turn:
            score += evaluate_piece(piece, square, end_game)
        else:
            score += -evaluate_piece(piece, square, end_game)

    #no_of_moves = len(list(board.legal_moves))*10
    # if not board.turn:
...
def minimax(board, depth, alpha, beta):
    ...
    if opening_move:
        return 0, reader.choice(board).move
    ...
    if player(board):
        ...
        for move in get_ordered_moves(board):
            eval_board = minimax(result(board, move), depth-1, alpha, beta)[0]
            if eval_board > maxEval:
                maxEval = eval_board
                bestMax = move
